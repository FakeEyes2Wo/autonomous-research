...
正文中的新增文件、配置键、命令和 API 均是**拟新增契约**，不能在当前代码中直接调用。各任务列出的现有文件已检查；根目录没有 package.json，命令必须在对应 package 下运行。
...
审阅后建议直接从 **A1 存储与数据契约**开始，按三个批次验收。若采用并行实现，允许 A 与 B0 独立推进，但 B1 必须等待 A5 和 B0；不在未整合的两个 ResearchStore 上各做一套 RAG 接入。
