...
function denylistTokens(meta: PaperMetaLike): string[] {
...
export function filterDenylist(text: string, meta: PaperMetaLike): string {
  let result = text
  for (const token of denylistTokens(meta)) result = result.split(token).join('[REDACTED]')
  return result
}
...
export function detectLeakage(text: string, meta: PaperMetaLike): string[] {
  return containsDenylistedToken(text, meta)
    ? ['LEAKAGE_FAIL: target paper metadata leaked into research output']
    : []
}
...
export function assertNoLeakage(text: string, meta: PaperMetaLike): void {
  ...
  if (leaks.length > 0) {
    const error = new Error(leaks.join('; ')) as Error & { code?: string }
