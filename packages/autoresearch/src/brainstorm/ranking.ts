 ...
 * repeated values keeps phase methods to a single `ctx` argument.
 */
...
/**
 * Default ranking: ask the brainstorm role to score every candidate, then sort
 * by total score with deterministic tie-breakers.
 *
 * This is intentionally a separate strategy so the search phase can reuse the
 * same pluggable interface without changing the pipeline.
 */
