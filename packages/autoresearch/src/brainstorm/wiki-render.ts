...
export function renderPaperWiki(paper: PaperRecord): string {
  const lines = [
    ...
    '',
    `- id: ${paper.id}`,
    ...(paper.workId ? [`- work id: ${paper.workId}`] : []),
    ...(paper.documentVersionId ? [`- document version: ${paper.documentVersionId}`] : []),
    `- source verification: ${paper.sourceStatus ?? 'legacy_unverified'}`,
    '- source reading coverage: not recorded in this card; consult exposure receipts',
